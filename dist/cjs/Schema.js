            ...
            if (name != 'primary') {
                if (index.type == 'local') {
                    if (index.hash && index.hash != primary.hash) {
                        throw new Error_js_1.OneTableArgError(`LSI "${name}" should not define a hash attribute that is different to the primary index`);
                    }
                    if (index.sort == null) {
                    ...
                    if (index.type == null) {
                        ...
                        throw new Error_js_1.OneTableArgError(`Index "${name}" is missing a hash attribute`);
                    }
                }
            }
        }
